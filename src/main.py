...
class RF_Controller:
	...
	def process_data(self, data):
		...
		if status == self.CMD_IDLE:
			...
			logger.info("CMD_REQ_SER ========= ROOM " + str(room_id))
			if room_id >= 1 and room_id <= 6: 
				room = room_map[room_id-1]
				if room and room.id == id:
			...
			logger.info("CMD_PROCESSING ========= ROOM " + str(room_id))
			if room_id >= 1 and room_id <= 6: 
				room = room_map[room_id-1]
				if room:
					# logger.info(room.status)
			...
			logger.info("CMD_REQ_ID ========= ROOM " + str(room_id))
			...
			logger.info("CMD_REQ_ID_OK ========= ROOM " + str(room_id))
			if lcd_state == LCD_STATE_CONFIG:
				if room_id >= 1 and room_id <= 6: 
					room = room_map[room_id-1]
					room.id = id
